 ...
 *
 * mtreeget.c
 *	  fetch tuples from a MTree scan.
 *
 *
 ...
 *
 * IDENTIFICATION
 *	  src/backend/access/mtree/mtreeget.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
/*
 * mtreekillitems() -- set LP_DEAD state for items an indexscan caller has
 * told us were killed.
 *
 ...
 */
static void
mtreekillitems(IndexScanDesc scan)
{
	...
	mtreecheckpage(scan->indexRelation, buffer);
...
/*
 * mtreeindex_keytest() -- does this index tuple satisfy the scan key(s)?
 *
 ...
 */
static bool
mtreeindex_keytest(IndexScanDesc scan,
				  IndexTuple tuple,
				  Page page,
				  OffsetNumber offset,
				  bool *recheck_p,
...
{
	MTREEScanOpaque so = (MTREEScanOpaque) scan->opaque;
	MTREESTATE  *mtreestate = so->mtreestate;
	...
	*recheck_distances_p = false;
	 ...
	 * referenced page.
	 */
	if (MTreeTupleIsInvalid(tuple))
	{
		int			i;

		if (MTreePageIsLeaf(page))	/* shouldn't happen */
			elog(ERROR, "invalid MTree tuple found on leaf page");
	...
	while (keySize > 0)
	{
		...
		if (key->sk_flags & SK_ISNULL)
		{
			 ...
			 * values because of assumption in MTree: union (VAL, NULL) is VAL.
			 * But if on non-leaf page key IS NULL, then all children are
			 * NULL.
			 */
			if (key->sk_flags & SK_SEARCHNULL)
			{
				if (MTreePageIsLeaf(page) && !isNull)
					return false;
			}
			else
			{
		...
		else
		{
			...
			mtreedentryinit(mtreestate, key->sk_attno - 1, &de,
						   datum, r, page, offset,
						   false, isNull);

			/*
			 * Call the Consistent function to evaluate the test.  The
			 * arguments are the index datum (as a MTREEENTRY*), the comparison
...
/*
 * Scan all items on the MTree index page identified by *pageItem, and insert
 * them into the queue (or directly to output areas)
 *
 * scan: index scan we are executing
 * pageItem: search queue item identifying an index page to scan
 * myDistances: distances array associated with pageItem, or NULL at the root
 * tbm: if not NULL, mtreegetbitmap's output bitmap
 * ntids: if not NULL, mtreegetbitmap's output tuple counter
 *
 ...
 */
static void
mtreeScanPage(IndexScanDesc scan, MTREESearchItem *pageItem,
			 IndexOrderByDistance *myDistances, TIDBitmap *tbm, int64 *ntids)
{
	...
	Assert(!MTREESearchItemIsHeap(*pageItem));
	...
	if (!XLogRecPtrIsInvalid(pageItem->data.parentlsn) &&
		(MTreeFollowRight(page) ||
		 pageItem->data.parentlsn < MTreePageGetNSN(page)) &&
		opaque->rightlink != InvalidBlockNumber /* sanity check */ )
	{
		/* There was a page split, follow right link to add pages */
		MTREESearchItem *item;
		...
		/* Create new MTREESearchItem for the right sibling index page */
		item = palloc(SizeOfMTREESearchItem(scan->numberOfOrderBys));
	...
	for (i = FirstOffsetNumber; i <= maxoff; i = OffsetNumberNext(i))
	{
		...
		/*
		 * Must call mtreeindex_keytest in tempCxt, and clean up any leftover
		 * junk afterward.
		 */
		oldcxt = MemoryContextSwitchTo(so->mtreestate->tempCxt);

		match = mtreeindex_keytest(scan, it, page, i,
								  &recheck, &recheck_distances);

		MemoryContextSwitchTo(oldcxt);
		MemoryContextReset(so->mtreestate->tempCxt);

		/* Ignore tuple if it doesn't match */
		if (!match)
			continue;

		if (tbm && MTreePageIsLeaf(page))
		{
			/*
		...
		else if (scan->numberOfOrderBys == 0 && MTreePageIsLeaf(page))
		{
			/*
			...
			if (scan->xs_want_itup)
			{
				...
				so->pageData[so->nPageData].recontup =
					mtreeFetchTuple(mtreestate, r, it);
				MemoryContextSwitchTo(oldcxt);
			}
			so->nPageData++;
		}
		else
		{
			...
			/* Create new MTREESearchItem for this item */
			item = palloc(SizeOfMTREESearchItem(scan->numberOfOrderBys));

			if (MTreePageIsLeaf(page))
			{
				/* Creating heap-tuple MTREESearchItem */
				 ...
				 */
				if (scan->xs_want_itup)
					item->data.heap.recontup = mtreeFetchTuple(mtreestate, r, it);
			}
			else
			{
				/* Creating index-page MTREESearchItem */
 ...
 *
 * Returns a MTREESearchItem or NULL.  Caller must pfree item when done with it.
 */
static MTREESearchItem *
getNextMTREESearchItem(MTREEScanOpaque so)
{
	MTREESearchItem *item;

	if (!pairingheap_is_empty(so->queue))
	{
		item = (MTREESearchItem *) pairingheap_remove_first(so->queue);
	}
	else
	{
...
getNextNearest(IndexScanDesc scan)
{
	...
	do
	{
		MTREESearchItem *item = getNextMTREESearchItem(so);

		if (!item)
			break;

		if (MTREESearchItemIsHeap(*item))
		{
		...
		else
		{
			...
			mtreeScanPage(scan, item, item->distances, NULL, NULL);
		}
...
/*
 * mtreegettuple() -- Get the next tuple in the scan
 */
bool
mtreegettuple(IndexScanDesc scan, ScanDirection dir)
{
	MTREEScanOpaque so = (MTREEScanOpaque) scan->opaque;

	if (dir != ForwardScanDirection)
		elog(ERROR, "MTree only supports forward scan direction");
	...
	if (so->firstCall)
	{
		...
		fakeItem.blkno = MTREE_ROOT_BLKNO;
		memset(&fakeItem.data.parentlsn, 0, sizeof(MTreeNSN));
		mtreeScanPage(scan, &fakeItem, NULL, NULL, NULL);
	}
		...
		for (;;)
		{
			if (so->curPageData < so->nPageData)
			{
				if (scan->kill_prior_tuple && so->curPageData > 0)
				{
					...
					{
						MemoryContext oldCxt =
						MemoryContextSwitchTo(so->mtreestate->scanCxt);
			...
			do
			{
				...
				item = getNextMTREESearchItem(so);
				...
				/* save current item BlockNumber for next mtreekillitems() call */
...
/*
 * mtreegetbitmap() -- Get a bitmap of all heap tuple locations
 */
int64
mtreegetbitmap(IndexScanDesc scan, TIDBitmap *tbm)
{
	MTREEScanOpaque so = (MTREEScanOpaque) scan->opaque;
	int64		ntids = 0;
	MTREESearchItem fakeItem;
	...
	fakeItem.blkno = MTREE_ROOT_BLKNO;
	memset(&fakeItem.data.parentlsn, 0, sizeof(MTreeNSN));
	mtreeScanPage(scan, &fakeItem, NULL, tbm, &ntids);
	...
	for (;;)
	{
		...
		mtreeScanPage(scan, item, item->distances, tbm, &ntids);
