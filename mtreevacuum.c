 ...
 *
 * mtreevacuum.c
 *	  vacuuming routines for the postgres MTree index access method.
 *
 *
 ...
 *
 * IDENTIFICATION
 *	  src/backend/access/mtree/mtreevacuum.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
/* Working state needed by mtreebulkdelete */
typedef struct
{
...
static void mtreevacuumscan(IndexVacuumInfo *info, IndexBulkDeleteResult *stats,
						   IndexBulkDeleteCallback callback, void *callback_state);
static void mtreevacuumpage(MTreeVacState *vstate, BlockNumber blkno,
						   BlockNumber orig_blkno);
static void mtreevacuum_delete_empty_pages(IndexVacuumInfo *info,
										  MTreeVacState *vstate);
static bool mtreedeletepage(IndexVacuumInfo *info, IndexBulkDeleteResult *stats,
			   ...
			   IndexBulkDeleteCallback callback, void *callback_state)
{
	...
	mtreevacuumscan(info, stats, callback, callback_state);
...
mtreevacuumcleanup(IndexVacuumInfo *info, IndexBulkDeleteResult *stats)
{
	...
	/*
	 * If mtreebulkdelete was called, we need not do anything, just return the
	 * stats from the latest mtreebulkdelete call.  If it wasn't called, we
	 * still need to do a pass over the index, to obtain index statistics.
	 */
	if (stats == NULL)
	{
		stats = (IndexBulkDeleteResult *) palloc0(sizeof(IndexBulkDeleteResult));
		mtreevacuumscan(info, stats, NULL, NULL);
	}
...
/*
 * mtreevacuumscan --- scan the index for VACUUMing purposes
 *
 ...
 */
static void
mtreevacuumscan(IndexVacuumInfo *info, IndexBulkDeleteResult *stats,
			   IndexBulkDeleteCallback callback, void *callback_state)
{
	...
	/* Set up info to pass down to mtreevacuumpage */
	vstate.info = info;
	vstate.stats = stats;
	...
	if (RelationNeedsWAL(rel))
		...
		vstate.startNSN = mtreeGetFakeLSN(rel);
	 ...
	 * write-locked by mtreeNewBuffer().  If we manage to scan such a page
	 ...
	 * include the new page, or mtreeNewBuffer already has write lock on the
	...
	for (;;)
	{
		...
		for (; blkno < num_pages; blkno++)
			mtreevacuumpage(&vstate, blkno, blkno);
	}
	 ...
	 * they can be reused.
	 */
	mtreevacuum_delete_empty_pages(info, &vstate);
...
/*
 * mtreevacuumpage --- VACUUM one page
 *
 * This processes a single page for mtreebulkdelete().  In some cases we
 * must go back and re-examine previously-scanned pages; this routine
 * recurses when necessary to handle that case.
 *
 * blkno is the page to process.  orig_blkno is the highest block number
 * reached by the outer mtreevacuumscan loop (the same as blkno, unless we
 * are recursing to re-examine a previous page).
 */
static void
mtreevacuumpage(MTreeVacState *vstate, BlockNumber blkno, BlockNumber orig_blkno)
{
...
restart:
	...
	else if (MTreePageIsDeleted(page))
	{
		/* Already deleted, but can't recycle yet */
		vstate->stats->pages_deleted++;
	}
	else if (MTreePageIsLeaf(page))
	{
		...
		if ((MTreeFollowRight(page) ||
			 vstate->startNSN < MTreePageGetNSN(page)) &&
	...
	else
	{
		 ...
		 * no version number anywhere in a MTree index, so we don't know
		 ...
		 * later in mtreevacuum_delete_empty_pages(), when we search for
 ...
 */
static void
mtreevacuum_delete_empty_pages(IndexVacuumInfo *info, MTreeVacState *vstate)
{
	Relation	rel = info->index;
		   ...
		   intset_iterate_next(vstate->internal_page_set, &blkno))
	{
		...
		if (PageIsNew(page) || MTreePageIsDeleted(page) || MTreePageIsLeaf(page))
		{
			/*
		 ...
		 * tuple to the page, so that it is no longer empty.  mtreedeletepage()
		...
		for (int i = 0; i < ntodelete; i++)
		{
			...
			leafbuf = ReadBufferExtended(rel, MAIN_FORKNUM, leafs_to_delete[i],
										 RBM_NORMAL, info->strategy);
			LockBuffer(leafbuf, MTREE_EXCLUSIVE);
			mtreecheckpage(rel, leafbuf);

			LockBuffer(buffer, MTREE_EXCLUSIVE);
			if (mtreedeletepage(info, vstate->stats,
...
/*
 * mtreedeletepage takes a leaf page, and its parent, and tries to delete the
 ...
 */
static bool
mtreedeletepage(IndexVacuumInfo *info, IndexBulkDeleteResult *stats,
			   ...
			   Buffer leafBuffer)
{
	 ...
	 */
	if (PageIsNew(parentPage) || MTreePageIsDeleted(parentPage) ||
		MTreePageIsLeaf(parentPage))
	{
	...
	MTreePageSetDeleted(leafPage, txid);
	stats->pages_deleted++;
	...
	if (RelationNeedsWAL(info->index))
		recptr = mtreeXLogPageDelete(leafBuffer, txid, parentBuffer, downlink);
	else
		recptr = mtreeGetFakeLSN(info->index);
