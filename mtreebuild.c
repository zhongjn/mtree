 ...
 *
 * mtreebuild.c
 *	  build algorithm for MTree indexes implementation.
 *
 *
 ...
 *
 * IDENTIFICATION
 *	  src/backend/access/mtree/mtreebuild.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
typedef enum
{
	MTREE_BUFFERING_DISABLED,	/* in regular build mode and aren't going to
								 * switch */
	MTREE_BUFFERING_AUTO,		/* in regular build mode, but will switch to
								 * buffering build mode if the index grows too
								 * big */
	MTREE_BUFFERING_STATS,		/* gathering statistics of index tuple size
								 * before switching to the buffering build
								 * mode */
	MTREE_BUFFERING_ACTIVE		/* in buffering build mode */
} MTreeBufferingMode;

/* Working state for mtreebuild and its callback */
typedef struct
{
	Relation	indexrel;
	Relation	heaprel;
	 ...
	 */
	MTREEBuildBuffers *gfbb;
	HTAB	   *parentMap;

	MTreeBufferingMode bufferingMode;
} MTREEBuildState;

/* prototypes for private functions */
static void mtreeInitBuffering(MTREEBuildState *buildstate);
static int	calculatePagesPerBuffer(MTREEBuildState *buildstate, int levelStep);
static void mtreeBuildCallback(Relation index,
							  ItemPointer tid,
							  Datum *values,
							  bool *isnull,
							  bool tupleIsAlive,
							  void *state);
static void mtreeBufferingBuildInsert(MTREEBuildState *buildstate,
									 IndexTuple itup);
static bool mtreeProcessItup(MTREEBuildState *buildstate, IndexTuple itup,
							BlockNumber startblkno, int startlevel);
static BlockNumber mtreebufferinginserttuples(MTREEBuildState *buildstate,
											 Buffer buffer, int level,
											 IndexTuple *itup, int ntup, OffsetNumber oldoffnum,
											 BlockNumber parentblk, OffsetNumber downlinkoffnum);
static Buffer mtreeBufferingFindCorrectParent(MTREEBuildState *buildstate,
											 BlockNumber childblkno, int level,
											 BlockNumber *parentblk,
											 OffsetNumber *downlinkoffnum);
static void mtreeProcessEmptyingQueue(MTREEBuildState *buildstate);
static void mtreeEmptyAllBuffers(MTREEBuildState *buildstate);
static int	mtreeGetMaxLevel(Relation index);

static void mtreeInitParentMap(MTREEBuildState *buildstate);
static void mtreeMemorizeParent(MTREEBuildState *buildstate, BlockNumber child,
							   BlockNumber parent);
static void mtreeMemorizeAllDownlinks(MTREEBuildState *buildstate, Buffer parent);
static BlockNumber mtreeGetParent(MTREEBuildState *buildstate, BlockNumber child);

/*
 * Main entry point to MTree index build. Initially calls insert over and over,
...
mtreebuild(Relation heap, Relation index, IndexInfo *indexInfo)
{
	...
	if (index->rd_options)
	{
		...
		MTreeOptions *options = (MTreeOptions *) index->rd_options;

		if (options->buffering_mode == MTREE_OPTION_BUFFERING_ON)
			buildstate.bufferingMode = MTREE_BUFFERING_STATS;
		else if (options->buffering_mode == MTREE_OPTION_BUFFERING_OFF)
			buildstate.bufferingMode = MTREE_BUFFERING_DISABLED;
		else
			buildstate.bufferingMode = MTREE_BUFFERING_AUTO;
	...
	else
	{
		 ...
		 */
		buildstate.bufferingMode = MTREE_BUFFERING_AUTO;
		fillfactor = MTREE_DEFAULT_FILLFACTOR;
	}
	...
	buildstate.mtreestate = initMTREEstate(index);

	/*
	 * Create a temporary memory context that is reset once for each tuple
	 * processed.  (Note: we don't bother to make this a child of the
	 * mtreestate's scanCxt, so we have to delete it separately at the end.)
	 */
	buildstate.mtreestate->tempCxt = createTempMTreeContext();

	/* initialize the root page */
	buffer = mtreeNewBuffer(index);
	Assert(BufferGetBlockNumber(buffer) == MTREE_ROOT_BLKNO);
	 ...
	 * buffers.
	 */
	if (buildstate.bufferingMode == MTREE_BUFFERING_ACTIVE)
	{
		elog(DEBUG1, "all tuples processed, emptying buffers");
		mtreeEmptyAllBuffers(&buildstate);
		mtreeFreeBuildBuffers(buildstate.gfbb);
	}

	/* okay, all heap tuples are indexed */
	MemoryContextSwitchTo(oldcxt);
	MemoryContextDelete(buildstate.mtreestate->tempCxt);

	freeMTREEstate(buildstate.mtreestate);
 ...
 * to MTREE_BUFFERING_DISABLED, so that we don't bother to try the switch
 * anymore. Otherwise initializes the build buffers, and sets bufferingMode to
 * MTREE_BUFFERING_ACTIVE.
 */
static void
mtreeInitBuffering(MTREEBuildState *buildstate)
{
	...
	pageFreeSpace = BLCKSZ - SizeOfPageHeaderData - sizeof(MTREEPageOpaqueData)
		- sizeof(ItemIdData)
	...
	if (levelStep <= 0)
	{
		elog(DEBUG1, "failed to switch to buffered MTree build");
		buildstate->bufferingMode = MTREE_BUFFERING_DISABLED;
		return;
	}
	...
	/* Initialize MTREEBuildBuffers with these parameters */
	buildstate->gfbb = mtreeInitBuildBuffers(pagesPerBuffer, levelStep,
											mtreeGetMaxLevel(index));

	mtreeInitParentMap(buildstate);

	buildstate->bufferingMode = MTREE_BUFFERING_ACTIVE;

	elog(DEBUG1, "switched to buffered MTree build; level step = %d, pagesPerBuffer = %d",
 ...
 */
static int
calculatePagesPerBuffer(MTREEBuildState *buildstate, int levelStep)
{
 ...
 */
static void
mtreeBuildCallback(Relation index,
				  ItemPointer tid,
				  Datum *values,
				  bool *isnull,
				  bool tupleIsAlive,
				  void *state)
{
	...
	oldCtx = MemoryContextSwitchTo(buildstate->mtreestate->tempCxt);

	/* form an index tuple and point it at the heap tuple */
	itup = mtreeFormTuple(buildstate->mtreestate, index, values, isnull, true);
	itup->t_tid = *tid;

	if (buildstate->bufferingMode == MTREE_BUFFERING_ACTIVE)
	{
		/* We have buffers, so use them. */
		mtreeBufferingBuildInsert(buildstate, itup);
	}
	else
	{
		/*
		 * There's no buffers (yet). Since we already have the index relation
		 * locked, we call mtreedoinsert directly.
		 */
		mtreedoinsert(index, itup, buildstate->freespace,
					 buildstate->mtreestate, buildstate->heaprel, true);
	}
	...
	MemoryContextReset(buildstate->mtreestate->tempCxt);
	 ...
	 */
	if ((buildstate->bufferingMode == MTREE_BUFFERING_AUTO &&
 ...
 */
static void
mtreeBufferingBuildInsert(MTREEBuildState *buildstate, IndexTuple itup)
{
	/* Insert the tuple to buffers. */
	mtreeProcessItup(buildstate, itup, 0, buildstate->gfbb->rootlevel);

	/* If we filled up (half of a) buffer, process buffer emptying. */
	mtreeProcessEmptyingQueue(buildstate);
}
 ...
 */
static bool
mtreeProcessItup(MTREEBuildState *buildstate, IndexTuple itup,
				BlockNumber startblkno, int startlevel)
{
	MTREESTATE  *mtreestate = buildstate->mtreestate;
	MTREEBuildBuffers *gfbb = buildstate->gfbb;
	...
	for (;;)
	{
		...
		childoffnum = mtreechoose(indexrel, page, itup, mtreestate);
		iid = PageGetItemId(page, childoffnum);
		idxtuple = (IndexTuple) PageGetItem(page, iid);
		childblkno = ItemPointerGetBlockNumber(&(idxtuple->t_tid));

		if (level > 1)
			mtreeMemorizeParent(buildstate, childblkno, blkno);

		/*
		 * Check that the key representing the target child node is consistent
		 * with the key we're inserting. Update it if it's not.
		 */
		newtup = mtreegetadjusted(indexrel, idxtuple, itup, mtreestate);
		if (newtup)
		{
			blkno = mtreebufferinginserttuples(buildstate,
											  buffer,
											  level,
											  &newtup,
											  1,
											  ...
											  InvalidBlockNumber,
											  InvalidOffsetNumber);
			/* mtreebufferinginserttuples() released the buffer */
		}
		else
			UnlockReleaseBuffer(buffer);
	...
	if (LEVEL_HAS_BUFFERS(level, gfbb))
	{
		 ...
		 */
		MTREENodeBuffer *childNodeBuffer;

		/* Find the buffer or create a new one */
		childNodeBuffer = mtreeGetNodeBuffer(gfbb, mtreestate, blkno, level);

		/* Add index tuple to it */
		mtreePushItupToNodeBuffer(gfbb, childNodeBuffer, itup);
	...
	else
	{
		...
		buffer = ReadBuffer(indexrel, blkno);
		LockBuffer(buffer, MTREE_EXCLUSIVE);
		mtreebufferinginserttuples(buildstate, buffer, level,
 ...
 *
 * This is analogous with mtreeinserttuples() in the regular insertion code.
 *
 ...
 */
static BlockNumber
mtreebufferinginserttuples(MTREEBuildState *buildstate, Buffer buffer, int level,
						  IndexTuple *itup, int ntup, OffsetNumber oldoffnum,
						  BlockNumber parentblk, OffsetNumber downlinkoffnum)
{
	MTREEBuildBuffers *gfbb = buildstate->gfbb;
	...
	is_split = mtreeplacetopage(buildstate->indexrel,
							   buildstate->freespace,
							   buildstate->mtreestate,
							   buffer,
	 ...
	 * parent.
	 */
	if (is_split && BufferGetBlockNumber(buffer) == MTREE_ROOT_BLKNO)
	{
		Page		page = BufferGetPage(buffer);
		OffsetNumber off;
		OffsetNumber maxoff;
		...
		elog(DEBUG2, "splitting MTree root page, now %d levels deep", gfbb->rootlevel);
		...
		if (gfbb->rootlevel > 1)
		{
			...
			for (off = FirstOffsetNumber; off <= maxoff; off++)
			{
				...
				LockBuffer(childbuf, MTREE_SHARE);
				mtreeMemorizeAllDownlinks(buildstate, childbuf);
				 ...
				 * root block.
				 */
				mtreeMemorizeParent(buildstate, childblkno, MTREE_ROOT_BLKNO);
			}
		}
	}
	...
	if (splitinfo)
	{
		 ...
		 * mtreefinishsplit() in the regular insertion code, but the locking is
		...
		parentBuffer =
			mtreeBufferingFindCorrectParent(buildstate,
		 ...
		 * split too. mtreeRelocateBuildBuffersOnSplit() will also adjust the
		 ...
		 */
		mtreeRelocateBuildBuffersOnSplit(gfbb,
										buildstate->mtreestate,
										buildstate->indexrel,
										level,
										buffer, splitinfo);
		...
		{
			MTREEPageSplitInfo *splitinfo = lfirst(lc);

			/*
			 * Remember the parent of each new child page in our parent map.
			 * This assumes that the downlinks fit on the parent page. If the
			 * parent page is split, too, when we recurse up to insert the
			 * downlinks, the recursive mtreebufferinginserttuples() call will
			 * update the map again.
			 */
			if (level > 0)
				mtreeMemorizeParent(buildstate,
			 ...
			 */
			if (level > 1)
				mtreeMemorizeAllDownlinks(buildstate, splitinfo->buf);
		...
		mtreebufferinginserttuples(buildstate, parentBuffer, level + 1,
 ...
 * block.
 *
 * This function serves the same purpose as mtreeFindCorrectParent() during
 * normal index inserts, but this is simpler because we don't need to deal
 * with concurrent inserts.
 */
static Buffer
mtreeBufferingFindCorrectParent(MTREEBuildState *buildstate,
							   BlockNumber childblkno, int level,
							   ...
							   OffsetNumber *downlinkoffnum)
{
	...
	if (level > 0)
		parent = mtreeGetParent(buildstate, childblkno);
	else
	{
		/*
	...
	page = BufferGetPage(buffer);
	LockBuffer(buffer, MTREE_EXCLUSIVE);
	mtreecheckpage(buildstate->indexrel, buffer);
	maxoff = PageGetMaxOffsetNumber(page);
	 ...
	 * find it. During normal mtree insertions, it might've moved to another
 ...
 */
static void
mtreeProcessEmptyingQueue(MTREEBuildState *buildstate)
{
	MTREEBuildBuffers *gfbb = buildstate->gfbb;
	...
	{
		MTREENodeBuffer *emptyingNodeBuffer;

		/* Get node buffer from emptying stack. */
		emptyingNodeBuffer = (MTREENodeBuffer *) linitial(gfbb->bufferEmptyingQueue);
		...
		while (true)
		{
			...
			if (!mtreePopItupFromNodeBuffer(gfbb, emptyingNodeBuffer, &itup))
				break;
			 ...
			 */
			if (mtreeProcessItup(buildstate, itup, emptyingNodeBuffer->nodeBlocknum, emptyingNodeBuffer->level))
			{
				/*
 ...
 */
static void
mtreeEmptyAllBuffers(MTREEBuildState *buildstate)
{
	MTREEBuildBuffers *gfbb = buildstate->gfbb;
	MemoryContext oldCtx;
	int			i;
	...
	for (i = gfbb->buffersOnLevelsLen - 1; i >= 0; i--)
	{
		...
		{
			MTREENodeBuffer *nodeBuffer;

			nodeBuffer = (MTREENodeBuffer *) linitial(gfbb->buffersOnLevels[i]);
			...
			if (nodeBuffer->blocksCount != 0)
			{
				...
				if (!nodeBuffer->queuedForEmptying)
				{
					...
					MemoryContextSwitchTo(buildstate->mtreestate->tempCxt);
				}
				mtreeProcessEmptyingQueue(buildstate);
			}
			else
...
/*
 * Get the depth of the MTree index.
 */
static int
mtreeGetMaxLevel(Relation index)
{
	int			maxLevel;
	BlockNumber blkno;
...
static void
mtreeInitParentMap(MTREEBuildState *buildstate)
{
	HASHCTL		hashCtl;
	...
	buildstate->parentMap = hash_create("mtreebuild parent map",
										1024,
										&hashCtl,
...
static void
mtreeMemorizeParent(MTREEBuildState *buildstate, BlockNumber child, BlockNumber parent)
{
 ...
 */
static void
mtreeMemorizeAllDownlinks(MTREEBuildState *buildstate, Buffer parentbuf)
{
	OffsetNumber maxoff;
	OffsetNumber off;
	...
	for (off = FirstOffsetNumber; off <= maxoff; off++)
	{
		...
		mtreeMemorizeParent(buildstate, childblkno, parentblkno);
	}
}

static BlockNumber
mtreeGetParent(MTREEBuildState *buildstate, BlockNumber child)
{
	ParentMapEntry *entry;
	bool		found;
