 ...
 *
 * mtreebuildbuffers.c
 *	  node buffer management functions for MTree buffering build algorithm.
 *
 *
 ...
 *
 * IDENTIFICATION
 *	  src/backend/access/mtree/mtreebuildbuffers.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
static MTREENodeBufferPage *mtreeAllocateNewPageBuffer(MTREEBuildBuffers *gfbb);
static void mtreeAddLoadedBuffer(MTREEBuildBuffers *gfbb,
								MTREENodeBuffer *nodeBuffer);
static void mtreeLoadNodeBuffer(MTREEBuildBuffers *gfbb,
							   MTREENodeBuffer *nodeBuffer);
static void mtreeUnloadNodeBuffer(MTREEBuildBuffers *gfbb,
								 MTREENodeBuffer *nodeBuffer);
static void mtreePlaceItupToPage(MTREENodeBufferPage *pageBuffer,
								IndexTuple item);
static void mtreeGetItupFromPage(MTREENodeBufferPage *pageBuffer,
								IndexTuple *item);
static long mtreeBuffersGetFreeBlock(MTREEBuildBuffers *gfbb);
static void mtreeBuffersReleaseBlock(MTREEBuildBuffers *gfbb, long blocknum);
...
/*
 * Initialize MTree build buffers.
 */
MTREEBuildBuffers *
mtreeInitBuildBuffers(int pagesPerBuffer, int levelStep, int maxLevel)
{
	MTREEBuildBuffers *gfbb;
	HASHCTL		hashCtl;

	gfbb = palloc(sizeof(MTREEBuildBuffers));
	...
	hashCtl.entrysize = sizeof(MTREENodeBuffer);
	hashCtl.hcxt = CurrentMemoryContext;
	gfbb->nodeBuffersTab = hash_create("mtreebuildbuffers",
									   1024,
									   &hashCtl,
	...
	gfbb->loadedBuffers = (MTREENodeBuffer **) palloc(gfbb->loadedBuffersLen *
													 sizeof(MTREENodeBuffer *));
 ...
 */
MTREENodeBuffer *
mtreeGetNodeBuffer(MTREEBuildBuffers *gfbb, MTREESTATE *mtreestate,
				  BlockNumber nodeBlocknum, int level)
{
	...
	nodeBuffer = (MTREENodeBuffer *) hash_search(gfbb->nodeBuffersTab,
 ...
 */
static MTREENodeBufferPage *
mtreeAllocateNewPageBuffer(MTREEBuildBuffers *gfbb)
{
	MTREENodeBufferPage *pageBuffer;

	pageBuffer = (MTREENodeBufferPage *) MemoryContextAllocZero(gfbb->context,
															   BLCKSZ);
 ...
 */
static void
mtreeAddLoadedBuffer(MTREEBuildBuffers *gfbb, MTREENodeBuffer *nodeBuffer)
{
	...
	if (gfbb->loadedBuffersCount >= gfbb->loadedBuffersLen)
	{
		...
		gfbb->loadedBuffers = (MTREENodeBuffer **)
			repalloc(gfbb->loadedBuffers,
					 gfbb->loadedBuffersLen * sizeof(MTREENodeBuffer *));
	}
 ...
 */
static void
mtreeLoadNodeBuffer(MTREEBuildBuffers *gfbb, MTREENodeBuffer *nodeBuffer)
{
	/* Check if we really should load something */
	if (!nodeBuffer->pageBuffer && nodeBuffer->blocksCount > 0)
	{
		/* Allocate memory for page */
		nodeBuffer->pageBuffer = mtreeAllocateNewPageBuffer(gfbb);
		...
		mtreeBuffersReleaseBlock(gfbb, nodeBuffer->pageBlocknum);

		/* Mark node buffer as loaded */
		mtreeAddLoadedBuffer(gfbb, nodeBuffer);
		nodeBuffer->pageBlocknum = InvalidBlockNumber;
	}
}
 ...
 */
static void
mtreeUnloadNodeBuffer(MTREEBuildBuffers *gfbb, MTREENodeBuffer *nodeBuffer)
{
	...
	if (nodeBuffer->pageBuffer)
	{
		...
		/* Get free file block */
		blkno = mtreeBuffersGetFreeBlock(gfbb);
 ...
 */
void
mtreeUnloadNodeBuffers(MTREEBuildBuffers *gfbb)
{
	int			i;

	/* Unload all the buffers that have a page loaded in memory. */
	for (i = 0; i < gfbb->loadedBuffersCount; i++)
		mtreeUnloadNodeBuffer(gfbb, gfbb->loadedBuffers[i]);
 ...
 */
static void
mtreePlaceItupToPage(MTREENodeBufferPage *pageBuffer, IndexTuple itup)
{
 ...
 */
static void
mtreeGetItupFromPage(MTREENodeBufferPage *pageBuffer, IndexTuple *itup)
{
	IndexTuple	ptr;
	Size		itupsz;
 ...
 */
void
mtreePushItupToNodeBuffer(MTREEBuildBuffers *gfbb, MTREENodeBuffer *nodeBuffer,
						 IndexTuple itup)
{
	/*
	...
	if (!nodeBuffer->pageBuffer)
		mtreeLoadNodeBuffer(gfbb, nodeBuffer);
	...
	if (PAGE_NO_SPACE(nodeBuffer->pageBuffer, itup))
	{
		...
		PAGE_FREE_SPACE(nodeBuffer->pageBuffer) =
			BLCKSZ - MAXALIGN(offsetof(MTREENodeBufferPage, tupledata));
	...
	mtreePlaceItupToPage(nodeBuffer->pageBuffer, itup);
 ...
 */
bool
mtreePopItupFromNodeBuffer(MTREEBuildBuffers *gfbb, MTREENodeBuffer *nodeBuffer,
						  IndexTuple *itup)
{
	/*
	 ...
	 */
	mtreeGetItupFromPage(nodeBuffer->pageBuffer, itup);
	...
	if (PAGE_IS_EMPTY(nodeBuffer->pageBuffer))
	{
		...
		if (prevblkno != InvalidBlockNumber)
		{
			 ...
			 */
			mtreeBuffersReleaseBlock(gfbb, prevblkno);
		}
		else
		{
 ...
 */
static long
mtreeBuffersGetFreeBlock(MTREEBuildBuffers *gfbb)
{
	/*
 ...
 */
static void
mtreeBuffersReleaseBlock(MTREEBuildBuffers *gfbb, long blocknum)
{
	int			ndx;
 ...
 */
void
mtreeFreeBuildBuffers(MTREEBuildBuffers *gfbb)
{
	/* Close buffers file. */
...
typedef struct
{
	MTREEENTRY	entry[INDEX_MAX_KEYS];
	bool		isnull[INDEX_MAX_KEYS];
	MTREEPageSplitInfo *splitinfo;
	MTREENodeBuffer *nodeBuffer;
 ...
 */
void
mtreeRelocateBuildBuffersOnSplit(MTREEBuildBuffers *gfbb, MTREESTATE *mtreestate,
								...
								Buffer buffer, List *splitinfo)
{
	 ...
	 */
	Assert(blocknum != MTREE_ROOT_BLKNO);
	memcpy(&oldBuf, nodeBuffer, sizeof(MTREENodeBuffer));
	oldBuf.isTemp = true;
	...
	{
		MTREEPageSplitInfo *si = (MTREEPageSplitInfo *) lfirst(lc);
		MTREENodeBuffer *newNodeBuffer;
		 ...
		 * empty.
		 */
		newNodeBuffer = mtreeGetNodeBuffer(gfbb, mtreestate, BufferGetBlockNumber(si->buf), level);
	 ...
	 * that is not tied.
	 *
	 * The page searching logic is very similar to mtreechoose().
	 */
	while (mtreePopItupFromNodeBuffer(gfbb, &oldBuf, &itup))
	{
		...
		for (i = 0; i < splitPagesCount; i++)
		{
			...
			for (j = 0; j < r->rd_att->natts; j++)
			{
				...
				usize = mtreepenalty(mtreestate, j,
		...
		mtreePushItupToNodeBuffer(gfbb, targetBufferInfo->nodeBuffer, itup);

		/* Adjust the downlink for this page, if needed. */
		newtup = mtreegetadjusted(r, targetBufferInfo->splitinfo->downlink,
								 itup, mtreestate);
		if (newtup)
		{
			mtreeDeCompressAtt(mtreestate, r,
