 ...
 *
 * mtree.c
 *	  interface routines for the postgres MTree index access method.
 *
 *
 ...
 *
 * IDENTIFICATION
 *	  src/backend/access/mtree/mtree.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
static void mtreefixsplit(MTREEInsertState *state, MTREESTATE *mtreestate);
static bool mtreeinserttuple(MTREEInsertState *state, MTREEInsertStack *stack,
							MTREESTATE *mtreestate, IndexTuple tuple, OffsetNumber oldoffnum);
static bool mtreeinserttuples(MTREEInsertState *state, MTREEInsertStack *stack,
							 MTREESTATE *mtreestate,
							 IndexTuple *tuples, int ntup, OffsetNumber oldoffnum,
							 Buffer leftchild, Buffer rightchild,
							 bool unlockbuf, bool unlockleftchild);
static void mtreefinishsplit(MTREEInsertState *state, MTREEInsertStack *stack,
							MTREESTATE *mtreestate, List *splitinfo, bool unlockbuf);
static void mtreeprunepage(Relation rel, Page page, Buffer buffer,
						  Relation heapRel);
...
/*
 * MTree handler function: return IndexAmRoutine with access method parameters
 * and callbacks.
 */
Datum
mtreehandler(PG_FUNCTION_ARGS)
{
	...
	amroutine->amsupport = MTREENProcs;
...
/*
 * Create and return a temporary memory context for use by MTree. We
 ...
 * MTree code itself, to avoid the need to do some awkward manual
 * memory management.
 */
MemoryContext
createTempMTreeContext(void)
{
	return AllocSetContextCreate(CurrentMemoryContext,
								 "MTree temporary context",
								 ALLOCSET_DEFAULT_SIZES);
}

/*
 *	mtreebuildempty() -- build an empty mtree index in the initialization fork
 */
void
mtreebuildempty(Relation index)
{
	Buffer		buffer;
...
/*
 *	mtreeinsert -- wrapper for MTree tuple insertion.
 *
 *	  This is the public interface routine for tuple insertion in MTrees.
 *	  It doesn't do any work; just locks the relation and passes the buck.
 */
bool
mtreeinsert(Relation r, Datum *values, bool *isnull,
		   ItemPointer ht_ctid, Relation heapRel,
		   IndexUniqueCheck checkUnique,
		   IndexInfo *indexInfo)
{
	MTREESTATE  *mtreestate = (MTREESTATE *) indexInfo->ii_AmCache;
	IndexTuple	itup;
	MemoryContext oldCxt;

	/* Initialize MTREESTATE cache if first call in this statement */
	if (mtreestate == NULL)
	{
		oldCxt = MemoryContextSwitchTo(indexInfo->ii_Context);
		mtreestate = initMTREEstate(r);
		mtreestate->tempCxt = createTempMTreeContext();
		indexInfo->ii_AmCache = (void *) mtreestate;
		MemoryContextSwitchTo(oldCxt);
	}

	oldCxt = MemoryContextSwitchTo(mtreestate->tempCxt);

	itup = mtreeFormTuple(mtreestate, r,
						 values, isnull, true /* size is currently bogus */ );
	itup->t_tid = *ht_ctid;

	mtreedoinsert(r, itup, 0, mtreestate, heapRel, false);

	/* cleanup */
	MemoryContextSwitchTo(oldCxt);
	MemoryContextReset(mtreestate->tempCxt);
 ...
 * 'buffer' is the root page and it needs to be split, mtreeplacetopage()
 ...
 */
bool
mtreeplacetopage(Relation rel, Size freespace, MTREESTATE *mtreestate,
				Buffer buffer,
				...
				bool is_build)
{
	...
	bool		is_leaf = (MTreePageIsLeaf(page)) ? true : false;
	XLogRecPtr	recptr;
	int			i;
	bool		is_split;
	 ...
	 * the mess.
	 */
	if (MTreeFollowRight(page))
		elog(ERROR, "concurrent MTree page split was incomplete");
	...
	*splitinfo = NIL;
	 ...
	 * because the tuple vector passed to mtreeSplit won't include this tuple.
	 */
	is_split = mtreenospace(page, itup, ntup, oldoffnum, freespace);

	/*
	 * If leaf page is full, try at first to delete dead tuples. And then
	 * check again.
	 */
	if (is_split && MTreePageIsLeaf(page) && MTreePageHasGarbage(page))
	{
		mtreeprunepage(rel, page, buffer, heapRel);
		is_split = mtreenospace(page, itup, ntup, oldoffnum, freespace);
	}
	...
	if (is_split)
	{
		...
		is_rootsplit = (blkno == MTREE_ROOT_BLKNO);

		/*
		 * Form index tuples vector to split. If we're replacing an old tuple,
		 * remove the old version from the vector.
		 */
		itvec = mtreeextractpage(page, &tlen);
		...
		itvec = mtreejoinvector(itvec, &tlen, itup, ntup);
		dist = mtreeSplit(rel, page, itvec, tlen, mtreestate);
		...
		if (is_rootsplit)
			npage++;
		if (npage > MTREE_MAX_SPLIT_PAGES)
			elog(ERROR, "MTree page split into too many halves (%d, maximum %d)",
				 npage, MTREE_MAX_SPLIT_PAGES);
		...
		if (!is_rootsplit)
		{
			...
			oldrlink = MTreePageGetOpaque(page)->rightlink;
			oldnsn = MTreePageGetNSN(page);
			...
			MTreePageGetOpaque(dist->page)->flags = (is_leaf) ? F_LEAF : 0;
		...
		{
			/* Allocate new page */
			ptr->buffer = mtreeNewBuffer(rel);
			MTREEInitBuffer(ptr->buffer, (is_leaf) ? F_LEAF : 0);
		...
		if (is_rootsplit)
		{
			...
			MTreePageGetOpaque(rootpg.page)->flags = 0;
			...
			rootpg.block.blkno = MTREE_ROOT_BLKNO;
			rootpg.block.num = ndownlinks;
			rootpg.list = mtreefillitupvec(downlinks, ndownlinks,
										  &(rootpg.lenlist));
			rootpg.itup = NULL;
		...
		else
		{
			...
			for (ptr = dist; ptr; ptr = ptr->next)
			{
				MTREEPageSplitInfo *si = palloc(sizeof(MTREEPageSplitInfo));
		...
		for (ptr = dist; ptr; ptr = ptr->next)
		{
			...
			/* Set up rightlinks */
			if (ptr->next && ptr->block.blkno != MTREE_ROOT_BLKNO)
				MTreePageGetOpaque(ptr->page)->rightlink =
					ptr->next->block.blkno;
			else
				MTreePageGetOpaque(ptr->page)->rightlink = oldrlink;
			...
			if (ptr->next && !is_rootsplit && markfollowright)
				MTreeMarkFollowRight(ptr->page);
			else
				MTreeClearFollowRight(ptr->page);
			 ...
			 */
			MTreePageSetNSN(ptr->page, oldnsn);
		}

		/*
		 * mtreeXLogSplit() needs to WAL log a lot of pages, prepare WAL
		 * insertion for that. NB: The number of pages and data segments
		 * specified here must match the calculations in mtreeXLogSplit()!
		 */
		 ...
		 */
		if (is_build)
			recptr = MTreeBuildLSN;
		else
		{
			if (RelationNeedsWAL(rel))
				recptr = mtreeXLogSplit(is_leaf,
									   dist, oldrlink, oldnsn, leftchildbuf,
									   markfollowright);
			else
				recptr = mtreeGetFakeLSN(rel);
		}
	...
	else
	{
		...
		if (OffsetNumberIsValid(oldoffnum))
		{
			...
			else
			{
				...
				mtreefillbuffer(page, itup, ntup, InvalidOffsetNumber);
			}
		}
		else
		{
	...
	if (BufferIsValid(leftchildbuf))
	{
		...
		MTreePageSetNSN(leftpg, recptr);
		MTreeClearFollowRight(leftpg);
...
/*
 * Workhouse routine for doing insertion into a MTree index. Note that
 * this routine assumes it is invoked in a short-lived memory context,
 * so it does not bother releasing palloc'd allocations.
 */
void
mtreedoinsert(Relation r, IndexTuple itup, Size freespace,
			 MTREESTATE *mtreestate, Relation heapRel, bool is_build)
{
	ItemId		iid;
	IndexTuple	idxtuple;
	MTREEInsertStack firststack;
	MTREEInsertStack *stack;
	MTREEInsertState state;
	bool		xlocked = false;

	memset(&state, 0, sizeof(MTREEInsertState));
	...
	/* Start from the root */
	firststack.blkno = MTREE_ROOT_BLKNO;
	firststack.lsn = 0;
	...
	for (;;)
	{
		...
		{
			if (xlocked)
				LockBuffer(stack->buffer, MTREE_UNLOCK);
			xlocked = false;
		...
		if (!xlocked)
		{
			LockBuffer(stack->buffer, MTREE_SHARE);
			mtreecheckpage(state.r, stack->buffer);
		}
		 ...
		 * that now.
		 */
		if (MTreeFollowRight(stack->page))
		{
			if (!xlocked)
			{
				LockBuffer(stack->buffer, MTREE_UNLOCK);
				LockBuffer(stack->buffer, MTREE_EXCLUSIVE);
				xlocked = true;
				/* someone might've completed the split when we unlocked */
				if (!MTreeFollowRight(stack->page))
					continue;
			}
			mtreefixsplit(&state, mtreestate);
		...
		if ((stack->blkno != MTREE_ROOT_BLKNO &&
			 stack->parent->lsn < MTreePageGetNSN(stack->page)) ||
			MTreePageIsDeleted(stack->page))
		{
			/*
		...
		if (!MTreePageIsLeaf(stack->page))
		{
			...
			IndexTuple	newtup;
			MTREEInsertStack *item;
			OffsetNumber downlinkoffnum;

			downlinkoffnum = mtreechoose(state.r, stack->page, itup, mtreestate);
			 ...
			 */
			if (MTreeTupleIsInvalid(idxtuple))
				ereport(ERROR,
			 ...
			 */
			newtup = mtreegetadjusted(state.r, idxtuple, itup, mtreestate);
			if (newtup)
			{
				/*
				 ...
				 * Update the tuple.
				 *
				 * We still hold the lock after mtreeinserttuple(), but it
				 ...
				 * tuple.
				 */
				if (mtreeinserttuple(&state, stack, mtreestate, newtup,
									downlinkoffnum))
				{
					/*
			...
			item = (MTREEInsertStack *) palloc0(sizeof(MTREEInsertStack));
		...
		else
		{
			 ...
			 * it doesn't fit. mtreeinserttuple() will take care of that.
			 */
			...
			if (!xlocked)
			{
				...
				else if ((MTreeFollowRight(stack->page) ||
						  stack->parent->lsn < MTreePageGetNSN(stack->page)) &&
						 MTreePageIsDeleted(stack->page))
				{
					/*
			...
			mtreeinserttuple(&state, stack, mtreestate, itup,
							InvalidOffsetNumber);
			LockBuffer(stack->buffer, MTREE_UNLOCK);
 ...
 */
static MTREEInsertStack *
mtreeFindPath(Relation r, BlockNumber child, OffsetNumber *downlinkoffnum)
{
	Page		page;
	Buffer		buffer;
	OffsetNumber i,
	...
	IndexTuple	idxtuple;
	List	   *fifo;
	MTREEInsertStack *top,
			   *ptr;
	BlockNumber blkno;

	top = (MTREEInsertStack *) palloc0(sizeof(MTREEInsertStack));
	top->blkno = MTREE_ROOT_BLKNO;
	...
	while (fifo != NIL)
	{
		...
		if (top->parent && top->parent->lsn < MTreePageGetNSN(page) &&
			MTreePageGetOpaque(page)->rightlink != InvalidBlockNumber /* sanity check */ )
		{
			/*
			 ...
			 */
			ptr = (MTREEInsertStack *) palloc0(sizeof(MTREEInsertStack));
			ptr->blkno = MTreePageGetOpaque(page)->rightlink;
 ...
 */
static void
mtreeFindCorrectParent(Relation r, MTREEInsertStack *child)
{
	MTREEInsertStack *parent = child->parent;

	mtreecheckpage(r, parent->buffer);
		...
		parent->lsn != PageGetLSN(parent->page))
	{
		...
		while (true)
		{
			...
			parent->blkno = MTreePageGetOpaque(parent->page)->rightlink;
			...
			LockBuffer(parent->buffer, MTREE_EXCLUSIVE);
			mtreecheckpage(r, parent->buffer);
			parent->page = (Page) BufferGetPage(parent->buffer);
		}
		...
		/* ok, find new path */
		ptr = parent = mtreeFindPath(r, child->blkno, &child->downlinkoffnum);

		/* read all buffers as expected by caller */
		/* note we don't lock them or mtreecheckpage them here! */
		while (ptr)
		{
		...
		LockBuffer(child->parent->buffer, MTREE_EXCLUSIVE);
		mtreeFindCorrectParent(r, child);
	}
}
 ...
 */
static IndexTuple
mtreeformdownlink(Relation rel, Buffer buf, MTREESTATE *mtreestate,
				 MTREEInsertStack *stack)
{
	...
	for (offset = FirstOffsetNumber; offset <= maxoff; offset = OffsetNumberNext(offset))
	{
		...
		else
		{
			...
			newdownlink = mtreegetadjusted(rel, downlink, ituple,
										  mtreestate);
			if (newdownlink)
	...
	if (!downlink)
	{
		...
		LockBuffer(stack->parent->buffer, MTREE_EXCLUSIVE);
		mtreeFindCorrectParent(rel, stack);
		iid = PageGetItemId(stack->parent->page, stack->downlinkoffnum);
		downlink = (IndexTuple) PageGetItem(stack->parent->page, iid);
		downlink = CopyIndexTuple(downlink);
		LockBuffer(stack->parent->buffer, MTREE_UNLOCK);
	}

	ItemPointerSetBlockNumber(&(downlink->t_tid), BufferGetBlockNumber(buf));
	MTreeTupleSetValid(downlink);
 ...
 */
static void
mtreefixsplit(MTREEInsertState *state, MTREESTATE *mtreestate)
{
	MTREEInsertStack *stack = state->stack;
	Buffer		buf;
	Page		page;
	List	   *splitinfo = NIL;
	...
	Assert(MTreeFollowRight(stack->page));
	...
	for (;;)
	{
		...
		downlink = mtreeformdownlink(state->r, buf, mtreestate, stack);
		...
		{
			/* lock next page */
			buf = ReadBuffer(state->r, MTreePageGetOpaque(page)->rightlink);
			LockBuffer(buf, MTREE_EXCLUSIVE);
		}
		else
			break;
	}
	...
	mtreefinishsplit(state, stack, mtreestate, splitinfo, false);
}
 ...
 */
static bool
mtreeinserttuple(MTREEInsertState *state, MTREEInsertStack *stack,
				MTREESTATE *mtreestate, IndexTuple tuple, OffsetNumber oldoffnum)
{
	return mtreeinserttuples(state, stack, mtreestate, &tuple, 1, oldoffnum,
							InvalidBuffer, InvalidBuffer, false, false);
}

/* ----------------
 * An extended workhorse version of mtreeinserttuple(). This version allows
 ...
 */
static bool
mtreeinserttuples(MTREEInsertState *state, MTREEInsertStack *stack,
				 MTREESTATE *mtreestate,
				 IndexTuple *tuples, int ntup, OffsetNumber oldoffnum,
				 Buffer leftchild, Buffer rightchild,
				 bool unlockbuf, bool unlockleftchild)
{
	...
	is_split = mtreeplacetopage(state->r, state->freespace, mtreestate,
							   stack->buffer,
							   tuples, ntup,
							   oldoffnum, NULL,
							   leftchild,
	...
	if (BufferIsValid(leftchild) && unlockleftchild)
		LockBuffer(leftchild, MTREE_UNLOCK);
	 ...
	 * mtreefinishsplit() to do that as soon as it's safe to do so. If we
	 * didn't have to split, release it ourselves.
	 */
	if (splitinfo)
		mtreefinishsplit(state, stack, mtreestate, splitinfo, unlockbuf);
	else if (unlockbuf)
		LockBuffer(stack->buffer, MTREE_UNLOCK);
 ...
 */
static void
mtreefinishsplit(MTREEInsertState *state, MTREEInsertStack *stack,
				MTREESTATE *mtreestate, List *splitinfo, bool unlockbuf)
{
	MTREEPageSplitInfo *right;
	MTREEPageSplitInfo *left;
	IndexTuple	tuples[2];
	...
	{
		right = (MTREEPageSplitInfo *) list_nth(splitinfo, pos);
		left = (MTREEPageSplitInfo *) list_nth(splitinfo, pos - 1);

		mtreeFindCorrectParent(state->r, stack);
		if (mtreeinserttuples(state, stack->parent, mtreestate,
							 &right->downlink, 1,
							 InvalidOffsetNumber,
		...
		/* mtreeinserttuples() released the lock on right->buf. */
	}

	right = (MTREEPageSplitInfo *) lsecond(splitinfo);
	left = (MTREEPageSplitInfo *) linitial(splitinfo);
		  ...
		  MTREESTATE *mtreestate)
{
		...
		ereport(ERROR,
				(errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
				 errmsg("index row size %zu exceeds maximum %zu for index \"%s\"",
						IndexTupleSize(itup[0]), MTreePageSize,
						RelationGetRelationName(r))));

	memset(v.spl_lisnull, true,
		   sizeof(bool) * mtreestate->nonLeafTupdesc->natts);
	memset(v.spl_risnull, true,
		   sizeof(bool) * mtreestate->nonLeafTupdesc->natts);
	mtreeSplitByKey(r, page, itup, len, mtreestate, &v, 0);
	...
	if (!mtreefitpage(rvectup, v.splitVector.spl_nright))
	{
		res = mtreeSplit(r, page, rvectup, v.splitVector.spl_nright, mtreestate);
	}
	else
	{
		ROTATEDIST(res);
		res->block.num = v.splitVector.spl_nright;
		res->list = mtreefillitupvec(rvectup, v.splitVector.spl_nright, &(res->lenlist));
		res->itup = mtreeFormTuple(mtreestate, r, v.spl_rattr, v.spl_risnull, false);
	}

	if (!mtreefitpage(lvectup, v.splitVector.spl_nleft))
	{
		SplitedPageLayout *resptr,
				   *subres;

		resptr = subres = mtreeSplit(r, page, lvectup, v.splitVector.spl_nleft, mtreestate);
	...
	else
	{
		...
		res->list = mtreefillitupvec(lvectup, v.splitVector.spl_nleft, &(res->lenlist));
		res->itup = mtreeFormTuple(mtreestate, r, v.spl_lattr, v.spl_lisnull, false);
	}
...
/*
 * Create a MTREESTATE and fill it with information about the index
 */
MTREESTATE *
initMTREEstate(Relation index)
{
	MTREESTATE  *mtreestate;
	...
	/* safety check to protect fixed-size arrays in MTREESTATE */
	if (index->rd_att->natts > INDEX_MAX_KEYS)
		elog(ERROR, "numberOfAttributes %d > %d",
			 index->rd_att->natts, INDEX_MAX_KEYS);

	/* Create the memory context that will hold the MTREESTATE */
	scanCxt = AllocSetContextCreate(CurrentMemoryContext,
									"MTree scan context",
									ALLOCSET_DEFAULT_SIZES);
	oldCxt = MemoryContextSwitchTo(scanCxt);

	/* Create and fill in the MTREESTATE */
	mtreestate = (MTREESTATE *) palloc(sizeof(MTREESTATE));

	mtreestate->scanCxt = scanCxt;
	mtreestate->tempCxt = scanCxt;	/* caller must change this if needed */
	mtreestate->leafTupdesc = index->rd_att;
	 ...
	 * internal pages the way MTree does.
	 */
	mtreestate->nonLeafTupdesc = CreateTupleDescCopyConstr(index->rd_att);
	mtreestate->nonLeafTupdesc->natts =
		IndexRelationGetNumberOfKeyAttributes(index);

	Assert(IndexRelationGetNumberOfKeyAttributes(index) == 1);
	fmgr_info_copy(&(mtreestate->distanceFn),
					index_getprocinfo(index, 1, MTREE_DISTANCE_PROC),
					scanCxt);

	// for (i = 0; i < IndexRelationGetNumberOfKeyAttributes(index); i++)
	// {
	// 	fmgr_info_copy(&(mtreestate->consistentFn[i]),
	// 				   index_getprocinfo(index, i + 1, MTREE_CONSISTENT_PROC),
	// 				   scanCxt);
	// 	fmgr_info_copy(&(mtreestate->unionFn[i]),
	// 				   index_getprocinfo(index, i + 1, MTREE_UNION_PROC),
	// 				   scanCxt);

	// 	/* opclasses are not required to provide a Compress method */
	// 	if (OidIsValid(index_getprocid(index, i + 1, MTREE_COMPRESS_PROC)))
	// 		fmgr_info_copy(&(mtreestate->compressFn[i]),
	// 					   index_getprocinfo(index, i + 1, MTREE_COMPRESS_PROC),
	// 					   scanCxt);
	// 	else
	// 		mtreestate->compressFn[i].fn_oid = InvalidOid;

	// 	/* opclasses are not required to provide a Decompress method */
	// 	if (OidIsValid(index_getprocid(index, i + 1, MTREE_DECOMPRESS_PROC)))
	// 		fmgr_info_copy(&(mtreestate->decompressFn[i]),
	// 					   index_getprocinfo(index, i + 1, MTREE_DECOMPRESS_PROC),
	// 					   scanCxt);
	// 	else
	// 		mtreestate->decompressFn[i].fn_oid = InvalidOid;

	// 	fmgr_info_copy(&(mtreestate->penaltyFn[i]),
	// 				   index_getprocinfo(index, i + 1, MTREE_PENALTY_PROC),
	// 				   scanCxt);
	// 	fmgr_info_copy(&(mtreestate->picksplitFn[i]),
	// 				   index_getprocinfo(index, i + 1, MTREE_PICKSPLIT_PROC),
	// 				   scanCxt);
	// 	fmgr_info_copy(&(mtreestate->equalFn[i]),
	// 				   index_getprocinfo(index, i + 1, MTREE_EQUAL_PROC),
	// 				   scanCxt);

	// 	/* opclasses are not required to provide a Distance method */
	// 	if (OidIsValid(index_getprocid(index, i + 1, MTREE_DISTANCE_PROC)))
	// 		fmgr_info_copy(&(mtreestate->distanceFn[i]),
	// 					   index_getprocinfo(index, i + 1, MTREE_DISTANCE_PROC),
	// 					   scanCxt);
	// 	else
	// 		mtreestate->distanceFn[i].fn_oid = InvalidOid;

	// 	/* opclasses are not required to provide a Fetch method */
	// 	if (OidIsValid(index_getprocid(index, i + 1, MTREE_FETCH_PROC)))
	// 		fmgr_info_copy(&(mtreestate->fetchFn[i]),
	// 					   index_getprocinfo(index, i + 1, MTREE_FETCH_PROC),
	// 					   scanCxt);
	// 	else
	// 		mtreestate->fetchFn[i].fn_oid = InvalidOid;
	...
	// 	 * any cases where a MTree storage type has a nondefault collation.)
	// 	 */
	// 	if (OidIsValid(index->rd_indcollation[i]))
	// 		mtreestate->supportCollation[i] = index->rd_indcollation[i];
	// 	else
	// 		mtreestate->supportCollation[i] = DEFAULT_COLLATION_OID;
	// }

	// /* No opclass information for INCLUDE attributes */
	// for (; i < index->rd_att->natts; i++)
	// {
	// 	mtreestate->consistentFn[i].fn_oid = InvalidOid;
	// 	mtreestate->unionFn[i].fn_oid = InvalidOid;
	// 	mtreestate->compressFn[i].fn_oid = InvalidOid;
	// 	mtreestate->decompressFn[i].fn_oid = InvalidOid;
	// 	mtreestate->penaltyFn[i].fn_oid = InvalidOid;
	// 	mtreestate->picksplitFn[i].fn_oid = InvalidOid;
	// 	mtreestate->equalFn[i].fn_oid = InvalidOid;
	// 	mtreestate->distanceFn[i].fn_oid = InvalidOid;
	// 	mtreestate->fetchFn[i].fn_oid = InvalidOid;
	// 	mtreestate->supportCollation[i] = InvalidOid;
	// } 
...
void
freeMTREEstate(MTREESTATE *mtreestate)
{
	/* It's sufficient to delete the scanCxt */
	MemoryContextDelete(mtreestate->scanCxt);
}

/*
 * mtreeprunepage() -- try to remove LP_DEAD items from the given page.
 * Function assumes that buffer is exclusively locked.
 */
static void
mtreeprunepage(Relation rel, Page page, Buffer buffer, Relation heapRel)
{
	...
	if (ndeletable > 0)
	{
		...
		if (RelationNeedsWAL(rel))
		{
			...
			PageSetLSN(page, mtreeGetFakeLSN(rel));
