 ...
 *
 * mtreescan.c
 *	  routines to manage scans on MTree index relations
 *
 *
 ...
 *
 * IDENTIFICATION
 *	  src/backend/access/mtree/mtreescan.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
/*
 * Pairing heap comparison function for the MTREESearchItem queue
 */
static int
pairingheap_MTREESearchItem_cmp(const pairingheap_node *a, const pairingheap_node *b, void *arg)
{
	const MTREESearchItem *sa = (const MTREESearchItem *) a;
	const MTREESearchItem *sb = (const MTREESearchItem *) b;
	...
	if (MTREESearchItemIsHeap(*sa) && !MTREESearchItemIsHeap(*sb))
		return 1;
	if (!MTREESearchItemIsHeap(*sa) && MTREESearchItemIsHeap(*sb))
		return -1;
...
/*
 * Index AM API functions for scanning MTree indexes
 */

IndexScanDesc
mtreebeginscan(Relation r, int nkeys, int norderbys)
{
	IndexScanDesc scan;
	...
	/* First, set up a MTREESTATE with a scan-lifespan memory context */
	mtreestate = initMTREEstate(scan->indexRelation);

	/*
	 * Everything made below is in the scanCxt, or is a child of the scanCxt,
	 * so it'll all go away automatically in mtreeendscan.
	 */
	oldCxt = MemoryContextSwitchTo(mtreestate->scanCxt);

	/* initialize opaque data */
	so = (MTREEScanOpaque) palloc0(sizeof(MTREEScanOpaqueData));
	so->mtreestate = mtreestate;
	mtreestate->tempCxt = createTempMTreeContext();
	so->queue = NULL;
	so->queueCxt = mtreestate->scanCxt;	/* see mtreerescan */
	...
	/*
	 * All fields required for index-only scans are initialized in mtreerescan,
...
void
mtreerescan(IndexScanDesc scan, ScanKey key, int nkeys,
		   ScanKey orderbys, int norderbys)
{
	...
	{
		/* first time through */
		Assert(so->queueCxt == so->mtreestate->scanCxt);
		first_time = true;
	}
	else if (so->queueCxt == so->mtreestate->scanCxt)
	{
		/* second time through */
		so->queueCxt = AllocSetContextCreate(so->mtreestate->scanCxt,
											 "MTree queue context",
	...
	if (scan->xs_want_itup && !scan->xs_hitupdesc)
	{
		...
		so->mtreestate->fetchTupdesc = CreateTemplateTupleDesc(natts);
		for (attno = 1; attno <= nkeyatts; attno++)
		{
			TupleDescInitEntry(so->mtreestate->fetchTupdesc, attno, NULL,
		...
		{
			/* taking opcintype from mtreestate->tupdesc */
			TupleDescInitEntry(so->mtreestate->fetchTupdesc, attno, NULL,
							   TupleDescAttr(so->mtreestate->leafTupdesc,
											 attno - 1)->atttypid,
							   -1, 0);
		}
		scan->xs_hitupdesc = so->mtreestate->fetchTupdesc;

		/* Also create a memory context that will hold the returned tuples */
		so->pageDataCxt = AllocSetContextCreate(so->mtreestate->scanCxt,
												"MTree page data context",
												ALLOCSET_DEFAULT_SIZES);
	}

	/* create new, empty pairing heap for search queue */
	oldCxt = MemoryContextSwitchTo(so->queueCxt);
	so->queue = pairingheap_allocate(pairingheap_MTREESearchItem_cmp, scan);
	MemoryContextSwitchTo(oldCxt);
		...
		for (i = 0; i < scan->numberOfOrderBys; i++)
		{
			...
			if (!OidIsValid(finfo->fn_oid))
				elog(ERROR, "missing support function %d for attribute %d of index \"%s\"",
					 MTREE_DISTANCE_PROC, skey->sk_attno,
					 RelationGetRelationName(scan->indexRelation));

			/*
			 * Look up the datatype returned by the original ordering
			 * operator. MTree always uses a float8 for the distance function,
			 ...
			 */
			fmgr_info_copy(&(skey->sk_func), finfo, so->mtreestate->scanCxt);
...
void
mtreeendscan(IndexScanDesc scan)
{
	MTREEScanOpaque so = (MTREEScanOpaque) scan->opaque;

	/*
	 * freeMTREEstate is enough to clean up everything made by mtreebeginscan,
	 * as well as the queueCxt if there is a separate context for it.
	 */
	freeMTREEstate(so->mtreestate);
}
